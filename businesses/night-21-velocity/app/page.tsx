  ...
  if (loading && !selectedStudent) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center">
        <div className="text-blue-400 text-xl">Loading Velocity...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-950">
      ...
      <header className="border-b border-gray-800 bg-gray-900/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-500 border border-gray-700 rounded px-2 py-0.5">AI Academic Advisor</span>
          </div>
          <div className="flex items-center gap-4">
            <a href="/admin" className="text-sm text-gray-400 hover:text-blue-400 transition-colors">
              Admin Dashboard
            </a>
            {selectedStudent && (
              <select
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-gray-400 mt-1">
                    ...
                    {selectedStudent.minor ? ` / Minor: ${selectedStudent.minor}` : ""} &middot;{" "}
                    Expected {selectedStudent.expectedGraduation}
                  </p>
                </div>
                <div className="flex items-center gap-6">
                  <div className="text-center">
                    <div className="text-2xl font-bold text-blue-400">{selectedStudent.gpa}</div>
              ...
              <div className="mt-6">
                ...
                <div className="w-full bg-gray-800 rounded-full h-3">
                  <div
                    ...
                    style={{ width: `${recommendation.progressSummary.percentComplete}%` }}
                  />
                </div>
              </div>
            </div>
                    ...
                    <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
                      <p className="text-sm text-gray-300">
                        {recommendation.graduationTimeline.onTrack
                          ? `Great progress! You are on track to graduate by ${recommendation.graduationTimeline.estimatedGraduation}. Keep maintaining your course load of 15 credits per semester.`
                          : `You are currently behind schedule. Consider taking summer courses or increasing your semester course load to get back on track for your ${selectedStudent.expectedGraduation} target.`}
                      </p>
                    </div>
                    <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700">
                      <p className="text-sm text-gray-300">
                        ...
                        {selectedStudent.gpa >= 3.5
                          ? "you are in excellent academic standing. Consider applying for honors programs or research opportunities."
                          : selectedStudent.gpa >= 3.0
                          ? "you have solid academic standing. Focus on maintaining this while building practical skills."
                          : "consider utilizing tutoring services and office hours to strengthen your academic performance."}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {recommendation.recommendedCourses.map((rec, i) => (
                    <div key={i} className="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-blue-700 transition-colors">
                      <div className="flex items-start justify-between mb-3">
                        <div>
                          <span className="text-xs text-blue-400 font-mono">{rec.course.code}</span>
                          <h4 className="text-white font-semibold mt-1">{rec.course.name}</h4>
                        </div>
                        <span className="text-xs bg-gray-800 text-gray-300 px-2 py-1 rounded">{rec.course.credits} credits</span>
                      </div>
                      <p className="text-sm text-gray-400 mb-3">{rec.reasoning}</p>
                      <div className="flex flex-wrap gap-1.5">
                        {rec.course.skillsTaught.map((skill) => (
                          <span key={skill} className="text-xs bg-blue-950/50 text-blue-300 border border-blue-800/50 px-2 py-0.5 rounded">
                            {skill}
                          </span>
                        ))}
                      </div>
                ...
                <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedStudent.skills.map((skill) => (
                      <span key={skill} className="bg-blue-950/50 text-blue-300 border border-blue-800/50 px-3 py-1.5 rounded-lg text-sm font-medium">
                        {skill}
                      </span>
                    ))}
                  </div>
      ...
      <footer className="border-t border-gray-800 mt-16 py-8 text-center text-gray-500 text-sm">
        <p>Velocity AI Academic Advisor &middot; Empowering student success through data-driven guidance</p>
      </footer>
    </div>
  );
}
